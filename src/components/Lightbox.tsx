...
export const Lightbox = ({
  ...
  return <Dialog open={initialIndex !== null} onOpenChange={open => !open && onClose()}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content {...swipeHandlers} className="fixed inset-0 z-[1050] overflow-hidden text-left bg-transparent outline-none touch-action-pan-y" onOpenAutoFocus={e => e.preventDefault()}>
            ...
            <div className="lg-inner absolute w-full inset-0 whitespace-nowrap">
              ...
              <div className="lg-item inline-block absolute w-full h-full p-10 text-center z-[1060]">
                ...
                <div className="lg-img-wrap absolute inset-0 whitespace-nowrap text-[0] inline-block w-full h-full p-10">
                  <img src={currentImage?.src.original} alt={`Image ${currentIndex + 1}`} className="lg-object lg-image inline-block align-middle max-w-full max-h-full w-auto h-auto relative scale-[1.4] md:scale-100" />
                </div>
              </div>
            </div>
          </div>
